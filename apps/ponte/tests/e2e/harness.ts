...
export const sourceEntry = (
  table: "skills" | "subagents",
  source: string,
  ref?: string,
  subdir?: string,
): string => {
  const lines = [`[[${table}]]`, `source = ${JSON.stringify(source)}`];
  if (ref !== undefined) {
    lines.push(`ref = ${JSON.stringify(ref)}`);
  }
  if (subdir !== undefined) {
    lines.push(`subdir = ${JSON.stringify(subdir)}`);
  }
  return `${lines.join("\n")}\n`;
};
...
export class Home {
  ...
  async appendConfig(text: string): Promise<void> {
    const config = await this.readFileText(this.configPath("config.toml"));
    await this.writeFile(this.configPath("config.toml"), `${config}\n${text}`);
  }
